...
export const setPosition = (lat, long) => dispatch => {
    ...
    return fetch(`http://146.148.86.45/weather/${lat}/${long}/${Math.round((new Date()).valueOf()/1000)}`)
...
const fetchForecasts = address => dispatch => {
    ...
    return fetch(`http://146.148.86.45/latlong/${address}`)
      .then(response => response.json())
      .then(json => {
          console.log('latlong:', json);
          dispatch(receiveLocation(address, json))
        return fetch(`http://146.148.86.45/weather/${json.Lat}/${json.Long}/${Math.round((new Date()).valueOf()/1000)}`)
          .then(response => response.json())
          .then(json => {
